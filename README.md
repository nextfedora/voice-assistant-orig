...
- **Text-to-Speech (TTS) Voice (pyttsx3)**: The script uses `pyttsx3` for text-to-speech output. `pyttsx3` is a cross-platform library that interfaces with available system TTS engines (e.g., NSSpeechSynthesizer on macOS, SAPI5 on Windows, eSpeak on Linux). **Note: TTS output is now non-streaming; the assistant will speak the language model's entire response only after it has been fully generated.**
